    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-teal-50 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-ink mb-2">🪑 久坐提醒工具</h1>
          <p className="text-ink-muted">定时提醒起身活动，保护颈椎和腰椎健康</p>
        </div>

        {/* 统计卡片 */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-surface rounded-xl shadow-lg p-6">
            <div className="text-sm text-ink-muted mb-1">今日休息次数</div>
            <div className="text-3xl font-bold text-green-600 dark:text-green-400">{todayBreaks}</div>
            <div className="text-xs text-ink-muted mt-1">次</div>
          </div>

          <div className="bg-surface rounded-xl shadow-lg p-6">
            <div className="text-sm text-ink-muted mb-1">总会话数</div>
            <div className="text-3xl font-bold text-teal-600 dark:text-teal-400">{totalSessions}</div>
            <div className="text-xs text-ink-muted mt-1">次</div>
          </div>

          <div className="bg-surface rounded-xl shadow-lg p-6">
            <div className="text-sm text-ink-muted mb-1">提醒间隔</div>
            <div className="text-3xl font-bold text-blue-600 dark:text-blue-400">{interval}</div>
            <div className="text-xs text-ink-muted mt-1">分钟</div>
          </div>
        </div>

        {/* 计时器 */}
        <div className="bg-surface rounded-xl shadow-lg p-8 mb-6">
          <div className="text-center mb-6">
            <div className="text-6xl font-bold text-ink mb-2">
              {formatTime(timeLeft)}
            </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={resetToday}
              className="px-6 bg-surface-inset text-ink py-3 rounded-lg font-medium hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
            >
              重置
            </button>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <input
                ...
                className="flex-1"
              />
              <span className="text-lg font-semibold text-ink w-16 text-right">
                {interval} 分
              </span>
            </div>
        ...
        <div className="bg-surface rounded-xl shadow-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {STRETCH_EXERCISES.map((exercise, index) => (
              <div
                key={index}
                className="p-4 bg-gradient-to-br from-green-50 to-teal-50 rounded-lg border border-green-200 dark:border-green-800"
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="font-semibold text-ink">{exercise.name}</div>
                  <span className="text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 px-2 py-1 rounded-full">
                    {exercise.duration}
                  </span>
                </div>
                <div className="text-sm text-ink-muted">{exercise.description}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-6 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
          <h3 className="font-semibold text-green-800 dark:text-green-300 mb-2">💡 健康建议</h3>
