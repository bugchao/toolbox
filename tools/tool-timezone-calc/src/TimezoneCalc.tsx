...
function getOffset(tz: string): string {
  try {
    ...
    const diff = Math.round((local.getTime() - new Date(now.toLocaleString('en-US', { timeZone: 'UTC' })).getTime()) / 3600000)
...
export function TimezoneCalc() {
  const [now, setNow] = useState(new Date())
  const [cards, setCards] = useState<TzCard[]>([
    { tz: 'Asia/Shanghai' }, { tz: 'America/New_York' }, { tz: 'Europe/London' }, { tz: 'Asia/Tokyo' }
  ])
  ...
  const addCard = () => {
    ...
    setCards(prev => [...prev, { tz: addTz }])
  }

  const removeCard = (tz: string) => setCards(prev => prev.filter(c => c.tz !== tz))

  const getTzInfo = (tz: string) => TIMEZONES.find(t => t.tz === tz) || { label: tz, flag: '🌐' }

  return (
    <div className="max-w-2xl mx-auto p-6 space-y-6">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">时差计算器</h1>
      <p className="text-gray-500 dark:text-gray-400">多时区时间对比，支持自定义时间换算</p>
        ...
        {cards.map(card => {
          ...
          return (
            <div key={card.tz} className={`rounded-xl border p-4 relative ${
              daytime ? 'bg-amber-50 dark:bg-amber-900/10 border-amber-200 dark:border-amber-800' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700'
            }`}>
