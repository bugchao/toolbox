...
const ROLE_DEFS: RoleDef[] = [
  { id: 'villager', team: 'good', color: 'bg-sky-100 text-sky-700 border-sky-300 dark:bg-sky-900/30 dark:text-sky-300 dark:border-sky-700' },
  { id: 'werewolf', team: 'evil', color: 'bg-rose-100 text-rose-700 border-rose-300 dark:bg-rose-900/30 dark:text-rose-300 dark:border-rose-700' },
  { id: 'wolfKing', team: 'evil', color: 'bg-rose-200 text-rose-800 border-rose-400 dark:bg-rose-800/40 dark:text-rose-300 dark:border-rose-600' },
  { id: 'seer', team: 'good', color: 'bg-violet-100 text-violet-700 border-violet-300 dark:bg-violet-900/30 dark:text-violet-300 dark:border-violet-700' },
  { id: 'witch', team: 'good', color: 'bg-emerald-100 text-emerald-700 border-emerald-300 dark:bg-emerald-900/30 dark:text-emerald-300 dark:border-emerald-700' },
  { id: 'hunter', team: 'good', color: 'bg-amber-100 text-amber-700 border-amber-300 dark:bg-amber-900/30 dark:text-amber-300 dark:border-amber-700' },
  { id: 'guard', team: 'good', color: 'bg-cyan-100 text-cyan-700 border-cyan-300 dark:bg-cyan-900/30 dark:text-cyan-300 dark:border-cyan-700' },
  { id: 'idiot', team: 'good', color: 'bg-yellow-100 text-yellow-700 border-yellow-300 dark:bg-yellow-900/30 dark:text-yellow-300 dark:border-yellow-700' },
  { id: 'cupid', team: 'neutral', color: 'bg-pink-100 text-pink-700 border-pink-300 dark:bg-pink-900/30 dark:text-pink-300 dark:border-pink-700' },
]
          ...
          <button
            ...
            className={`group w-64 h-80 rounded-2xl border-2 shadow-2xl flex flex-col items-center justify-center transition-all ${
              data.deal.cardVisible
                ? 'bg-surface border-edge-strong text-ink'
          ...
          >
            {data.deal.cardVisible ? (
              <div className="space-y-4 px-4">
                ...
                <div className="text-xs text-ink-subtle pt-2">{t('reveal.tapToHide')}</div>
              </div>
            ) : (
              <>
    ...
    <div className="w-full space-y-6 pb-24">
      ...
      <section className="rounded-lg border border-edge bg-surface p-5 space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <h2 className="text-base font-semibold text-ink flex items-center gap-2">
            ...
            <span className="text-xs text-ink-subtle font-normal ml-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
          {ROLE_DEFS.map((def) => {
            ...
            return (
              <div
                key={def.id}
                className="flex items-center justify-between gap-2 rounded-md border border-edge px-3 py-2 hover:border-gray-300 dark:hover:border-gray-500 transition-colors"
              >
                ...
                <div className="inline-flex items-center border border-edge rounded-md overflow-hidden shrink-0">
                  <button
                    type="button"
                    onClick={() => incRole(def.id, -1)}
                    disabled={n <= 0}
                    className="px-2 py-1 text-ink-muted hover:bg-surface-muted disabled:text-gray-300 dark:disabled:text-gray-600 disabled:cursor-not-allowed"
                    aria-label={t('setup.decrement')}
                  >
                    <Minus className="w-3.5 h-3.5" />
                  </button>
                  <span className="px-2 py-1 text-sm font-mono text-ink min-w-[1.75rem] text-center tabular-nums">
                    {n}
                  </span>
                  <button
                    type="button"
        ...
        {balanceWarning && (
          <div className="flex items-start gap-2 text-xs text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-md px-3 py-2">
      ...
      {totalCount >= 4 && (
        <section className="rounded-lg border border-edge bg-surface p-5 space-y-3">
          <h3 className="text-sm font-medium text-ink-muted">{t('setup.namesLabel')}</h3>
          <p className="text-xs text-ink-subtle">{t('setup.namesHint')}</p>
