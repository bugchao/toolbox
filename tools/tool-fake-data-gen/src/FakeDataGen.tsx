  ...
  if (loading) {
    return (
      <div className="w-full space-y-6">
        ...
        <div className="text-center text-sm text-gray-400 dark:text-gray-500 py-12">{t('loading')}</div>
      </div>
    )
  }
    ...
    <div className="w-full space-y-6 pb-24">
      ...
      <section className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 space-y-3">
        <div className="flex flex-wrap items-center gap-3">
          <label className="flex items-center gap-2 text-sm">
            <span className="text-gray-700 dark:text-gray-300">{t('control.count')}:</span>
            <input
              type="number"
              min={1}
              max={10000}
          ...
          <label className="flex items-center gap-2 text-sm">
            <span className="text-gray-700 dark:text-gray-300">{t('control.format')}:</span>
            <select
              value={data.format}
          ...
          {data.format === 'sql' && (
            <label className="flex items-center gap-2 text-sm">
              <span className="text-gray-700 dark:text-gray-300">{t('control.table')}:</span>
              <input
                type="text"
                value={data.tableName}
                onChange={(e) => save({ ...data, tableName: e.target.value })}
                className="w-32 px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
            </label>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          {PRESETS.map((p) => (
            <button
              ...
              className="px-2 py-1 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:border-indigo-300 dark:hover:border-indigo-500 text-gray-700 dark:text-gray-300 transition-colors"
            >
              {isZh ? p.zh : p.en}
            </button>
          ))}
      ...
      <section className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 space-y-2">
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200">{t('schema.title')} ({data.fields.length})</h3>
          <button
            type="button"
            onClick={addField}
        ...
        {data.fields.length === 0 ? (
          <p className="text-xs text-gray-400 dark:text-gray-500 py-4 text-center">{t('schema.empty')}</p>
        ) : (
          <ul className="space-y-2">
            {data.fields.map((f, idx) => (
              <li
                key={f.id}
                className="flex flex-wrap items-center gap-2 rounded-md border border-gray-200 dark:border-gray-700 p-2 bg-gray-50 dark:bg-gray-700/30"
              >
                <div className="flex flex-col">
                  <button
                    ...
                    disabled={idx === 0}
                    className="text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 disabled:opacity-30 text-xs leading-none"
                  >
                    ▲
                  </button>
                ...
                <input
                  ...
                  className="w-32 px-2 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
                <select
                  value={f.type}
                  onChange={(e) => updateField(f.id, { type: e.target.value as FieldType, config: undefined })}
                  className="flex-1 min-w-[180px] px-2 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                >
      ...
      <section className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 space-y-2">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200">{t('output.title')}</h3>
          <div className="flex gap-1">
            <CopyButton
              ...
              className="!px-2 !py-1 text-xs border border-gray-200 dark:border-gray-700 rounded hover:border-indigo-300 dark:hover:border-indigo-500 hover:!text-indigo-600 dark:hover:!text-indigo-400"
            />
            <button
              type="button"
        ...
        <textarea
          ...
          className="w-full h-80 px-3 py-2 text-xs border border-gray-300 dark:border-gray-600 rounded-md font-mono bg-gray-50 dark:bg-gray-900/50 text-gray-900 dark:text-gray-100 resize-none"
          spellCheck={false}
        />
      </section>
...
const FieldConfigEditor: React.FC<FieldConfigEditorProps> = ({ field, onChange, t }) => {
  ...
  const numCls =
    'w-16 px-1.5 py-0.5 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 font-mono focus:outline-none focus:ring-1 focus:ring-indigo-500'
  if (
    field.type === 'integer' ||
    field.type === 'amount' ||
    field.type === 'float' ||
  ...
  if (field.type === 'enum') {
    return (
      <input
        ...
        className="flex-1 min-w-[140px] px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 font-mono focus:outline-none focus:ring-1 focus:ring-indigo-500"
      />
    )
  }
