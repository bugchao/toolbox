...
const ROLE_COLOR: Record<Role, string> = {
  civilian: 'bg-sky-100 text-sky-700 border-sky-300 dark:bg-sky-900/30 dark:text-sky-300 dark:border-sky-700',
  undercover: 'bg-rose-100 text-rose-700 border-rose-300 dark:bg-rose-900/30 dark:text-rose-300 dark:border-rose-700',
  whiteboard: 'bg-amber-100 text-amber-700 border-amber-300 dark:bg-amber-900/30 dark:text-amber-300 dark:border-amber-700',
}
...
const UndercoverGame: React.FC = () => {
      ...
      <div className="w-full space-y-6 pb-24">
        ...
        <section className="rounded-lg border border-edge bg-surface p-5 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-sm text-ink-muted block mb-1">
                ...
                <span className="text-xs text-ink-subtle">({t('setup.maxHint', { max: maxUnder })})</span>
              </label>
              <input
                type="range"
                min={1}
          ...
          <label className="inline-flex items-center gap-2 text-sm text-ink-muted cursor-pointer">
            ...
            <span className="text-xs text-ink-subtle">{t('setup.whiteboardHint')}</span>
          </label>

          {/* Player names (optional) */}
          <div className="space-y-2">
            <p className="text-sm text-ink-muted">{t('setup.playerNames')}</p>
          ...
          {customOpen && (
            <div className="px-4 pb-4 border-t border-edge pt-3 space-y-3">
              ...
              {data.customPairs.length > 0 ? (
                <ul className="divide-y divide-edge max-h-60 overflow-y-auto">
                  {data.customPairs.map((c) => (
                    <li key={c.id} className="py-2 flex items-center gap-3 text-sm">
                      <span className="font-mono text-ink">{c.civilian}</span>
                      <span className="text-ink-subtle">↔</span>
                      <span className="font-mono text-ink">{c.undercover}</span>
                      <button
                        type="button"
  ...
  if (game.stage === 'reveal') {
    ...
    return (
      <div className="w-full space-y-6">
        <PageHero title={t('title')} description={t('description')} />
        <section className="rounded-2xl border border-edge bg-gradient-to-br from-gray-50 to-white dark:from-gray-900 dark:to-gray-800 p-6 md:p-8 min-h-[360px] flex flex-col items-center justify-center text-center gap-4">
          ...
          >
            {game.revealVisible ? (
              <div className="space-y-4 px-4">
                ...
                <div className="text-xs text-ink-subtle">{t('reveal.tapToHide')}</div>
              </div>
            ) : (
              <>
        ...
        <div className="text-center">
          <button
            type="button"
            onClick={endGame}
            className="text-xs text-ink-subtle hover:text-red-500 inline-flex items-center gap-1"
          >
  ...
  return (
    <div className="w-full space-y-6 pb-24">
            ...
            return (
              <li
                ...
                className={`relative rounded-lg border px-3 py-2 text-sm transition-all ${
                  p.alive
                    ...
                    : 'bg-surface-muted border-edge opacity-60'
                } ${game.stage === 'vote' && p.alive ? 'cursor-pointer hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/30' : ''}`}
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className={`font-medium ${p.alive ? 'text-ink' : 'text-ink-muted line-through'}`}>
                    {p.name}
                  </span>
                  {!p.alive && <Skull className="w-3 h-3 text-ink-subtle ml-auto" />}
                </div>
                {showRole && (
                  <div className="mt-1 flex items-center gap-1">
                    ...
                    {p.word && <span className="text-xs text-ink-muted truncate">{p.word}</span>}
                  </div>
                )}
              </li>
            )
      ...
      {game.stage === 'discuss' && (
        <section className="rounded-lg border border-indigo-200 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-900/20 p-5 text-center space-y-3">
          <p className="text-sm text-indigo-700 dark:text-indigo-300">{t('discuss.hint')}</p>
          <button
            type="button"
            onClick={goToVote}
      ...
      {game.stage === 'vote' && (
        <section className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-5 text-center">
          <p className="text-sm text-amber-800 dark:text-amber-300">{t('vote.hint')}</p>
        </section>
      )}

      {game.stage === 'reveal-elim' && eliminatedPlayer && (
        <section className="rounded-2xl border border-edge bg-gradient-to-br from-rose-50 dark:from-rose-950/30 to-white dark:to-surface p-6 md:p-8 text-center space-y-4">
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            {eliminatedPlayer.word && (
              <span className="text-sm text-ink font-mono">"{eliminatedPlayer.word}"</span>
            )}
          </div>
          <button
            type="button"
      ...
      {game.stage === 'ended' && game.winner && (
        <section className="rounded-2xl border border-amber-200 dark:border-amber-800 bg-gradient-to-br from-amber-50 dark:from-amber-950/30 via-white dark:via-surface to-yellow-50 dark:to-yellow-950/30 p-6 md:p-8 text-center space-y-4">
          <Crown className="w-12 h-12 text-amber-500 mx-auto" />
          <h3 className="text-xl md:text-2xl font-bold text-ink flex items-center justify-center gap-2">
          ...
          <div className="flex flex-wrap gap-2 justify-center pt-2">
            ...
            <button
              type="button"
              onClick={endGame}
              className="px-4 py-2 text-sm font-medium bg-surface text-ink-muted rounded-full border border-edge-strong hover:bg-surface-muted transition-colors"
            >
              {t('winner.exit')}
            </button>
          </div>
