...
async function measureOne(provider: DohProvider): Promise<number> {
  const start = performance.now()
  const url = `${provider.url}?name=${encodeURIComponent(TEST_DOMAIN)}&type=A`
  await fetch(url, { headers: provider.needsJsonAccept ? { Accept: 'application/dns-json' } : {} })
  return Math.round(performance.now() - start)
}

export async function measureProvider(provider: DohProvider): Promise<LatencyResult> {
  const times: number[] = []
