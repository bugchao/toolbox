      ...
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-2 bg-surface-inset p-1 rounded-md">
            <button
              ...
              className={`px-4 py-2 rounded-md transition-colors ${
                mode === 'encode' ? 'bg-indigo-600 text-white' : 'text-ink hover:bg-gray-200 dark:hover:bg-gray-600'
              }`}
            >
              编码
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-md transition-colors ${
                mode === 'decode' ? 'bg-indigo-600 text-white' : 'text-ink hover:bg-gray-200 dark:hover:bg-gray-600'
              }`}
            >
              解码
            </button>
        ...
        <div>
          ...
          {mode === 'encode' && output.startsWith('data:image/') ? (
            <div className="p-4 border border-edge-strong rounded-md bg-surface-muted">
              <p className="text-sm text-ink-muted mb-2">图片预览:</p>
              <img src={output} alt="Base64 preview" className="max-h-64 max-w-full object-contain mb-4" />
              <textarea
                value={output}
                readOnly
                className="w-full h-32 p-4 border border-edge-strong rounded-md font-mono text-sm bg-surface focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
            </div>
          ) : (
            <textarea
